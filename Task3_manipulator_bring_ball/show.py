...
show.py: “机械手平面抓球并跨障送至目标位置“任务 — 训练结果(`outputs/`)演示与录制脚本
    ...
    2. 模型选择 (`Task3_manipulator_bring_ball/outputs/aaa/bbb/ccc`):
        - 示例: `--exp_name "v5_exp-03_PPO_r5" --choose_model "latest" --match_id 82.85`
        - `--exp_name` 参数：指定了实验目录名称 aaa(必须手动传入)
        - `--choose_model` 参数：加载对应实验目录, bbb=[best,latest,stages] 
        - `--match_id` 参数：通过文件名中的特定标识快速加载模型 ccc; 如步数'2002944'或奖励'23.94'；如果不指定，则默认选择 reward 最高的模型
    3. 泛化性测试：
        step1: 获取 train 阶段环境参数: mjpython Task3_manipulator_bring_ball/show.py --help
        step2: 测试 train 时没见过的 env 初始状态
        演示示例: mjpython Task3_manipulator_bring_ball/show.py --wall 0.020 --ball 0.300 0.032 --target -0.250 0.400 --exp_name "v5_exp-03_PPO_r5" --choose_model "latest" --match_id 82.85
"""
import os
import re
import glob
...
def get_args():
    ...
    parser.add_argument("--target", type=float, nargs=2, metavar=('X', 'Z'), 
                        help="默认随机；可手动指定目标的位置(如: --target -0.250 0.400)")
    
    # 运行配置
    parser.add_argument("--steps", type=int, default=1000, 
                        help="episode最大步数; 默认 1000 不建议更改")
