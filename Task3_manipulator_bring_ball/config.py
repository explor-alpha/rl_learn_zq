...
class TrainConfig:
    ...
    # --- 启动 CPU 并行运算 ---
    n_envs = 10  # WSL CPU核心 = 12 ；使用 10 个并行环境
    ...
    eval_freq_steps: int = (2048 * 10) * 5  # train.py， 更新 (n_steps * n_envs) * 5 次后调用 hook 评估训练效果
    total_timesteps: int = 1000000  # train.py，learn，total_timesteps后评估一次课程训练效果，判断是否进入下一课程或终止

    # --- PPO 超参 ---
    batch_size: int = 256   # 10 个环境并行。一次梯度更新拥有 n_steps * n_envs条数据。 256 依然属于中等偏小的尺寸，非常安全，不容易引起过拟合。
